#!/usr/bin/env python3
"""Chunk2 per-layer FP16 knockout analysis for Qwen3.5-4B.

Exports chunk2 (layers 7-10) with targeted per-layer fp16 precision via
half()->float() roundtrip casts baked into the traced graph, while the
overall compute_precision stays FLOAT32. Each variant is exported as a
proper dedup (infer+prefill multifunction) .mlpackage.

Configurations tested:
  1. chunk2_all_fp32     — full fp32 reference (all 9 dedup chunks from fp32)
  2. chunk2_all_fp16     — swap chunk2 from fp16 dedup
  3. layer7_fp16_only    — only layer 7 fp16-cast, rest fp32
  4. layer8_fp16_only    — only layer 8 fp16-cast, rest fp32
  5. layer9_fp16_only    — only layer 9 fp16-cast, rest fp32
  6. layer10_fp16_only   — only layer 10 fp16-cast, rest fp32
  7. layer7+8_fp16       — layers 7-8 fp16-cast
  8. layer8+9_fp16       — layers 8-9 fp16-cast
  9. layer9+10_fp16      — layers 9-10 fp16-cast

Usage:
    python tests/dev/chunk2_layer_knockout.py
    python tests/dev/chunk2_layer_knockout.py --skip-export
    python tests/dev/chunk2_layer_knockout.py --tokens 60
"""
...
KNOCKOUT_DIR = os.path.join(FP32_DIR, "knockout_layers")
...
# ======================================================================
#  Step 1: Export chunk2 variants with per-layer fp16 knockout
# ======================================================================

def _monkey_patch_fp16_layers(model_inner, fp16_layer_indices):
    """Monkey-patch the model's layer-processing loops to inject half()->float()
    roundtrip casts on hidden_states BEFORE and AFTER targeted layers.
    This simulates fp16 compute precision for those layers while keeping the
    overall traced graph in fp32.
    """
    fp16_set = set(fp16_layer_indices)

    # Patch the single-token (decode) loop
    orig_decode = model_inner.process_layers_regular_single_token_export_local_state

    def patched_decode(hidden_states, position_ids, causal_mask, current_pos,
                       kv_cache_0, k_cache=None, v_cache=None,
                       linear_conv_state=None, linear_recurrent_state=None,
                       start_layer=0, end_layer=None, apply_final_norm=False):
        if end_layer is None:
            end_layer = len(model_inner.layers)
        local_num_layers = end_layer - start_layer
        for local_layer_idx, layer_idx in enumerate(range(start_layer, end_layer)):
            if layer_idx in fp16_set:
                hidden_states = hidden_states.half().float()
            hidden_states = model_inner._process_layer_regular_single_token_export_local_state(
                layer_idx=layer_idx,
            ...
            if layer_idx in fp16_set:
                hidden_states = hidden_states.half().float()
        if apply_final_norm:
            hidden_states = model_inner.norm(hidden_states)
        return hidden_states

    model_inner.process_layers_regular_single_token_export_local_state = patched_decode

    # Patch the prefill loop
    orig_prefill = model_inner.process_layers_prefill_export_local_state

    def patched_prefill(hidden_states, position_ids, causal_mask, current_pos,
                        kv_cache_0, k_cache=None, v_cache=None,
                        linear_conv_state=None, linear_recurrent_state=None,
                        start_layer=0, end_layer=None, apply_final_norm=False,
                        expected_batch_size=None, expected_seq_len=None,
                        valid_len=None, mask_padding_hidden_states=False):
        if end_layer is None:
            end_layer = len(model_inner.layers)
        local_num_layers = end_layer - start_layer
        ...
        for local_layer_idx, layer_idx in enumerate(range(start_layer, end_layer)):
            if layer_idx in fp16_set:
                hidden_states = hidden_states.half().float()
            hidden_states = model_inner._process_layer_prefill_export_local_state(
                layer_idx=layer_idx,
                local_layer_idx=local_layer_idx,
                hidden_states=hidden_states,
                position_ids=position_ids,
    ...
    model_inner.process_layers_prefill_export_local_state = patched_prefill

    return orig_decode, orig_prefill


def _restore_model(model_inner, orig_decode, orig_prefill):
    """Undo monkey-patch."""
    model_inner.process_layers_regular_single_token_export_local_state = orig_decode
    model_inner.process_layers_prefill_export_local_state = orig_prefill


def export_knockout_chunk2(model, fp16_layers, label, skip_existing=False):
    """Export chunk2 as a dedup (infer+prefill) mlpackage with specific layers fp16-cast."""
    ...
    try:
        ...
        _restore_model(model.model, orig_dec, orig_pf)
...
def export_all_variants(skip_existing=False):
    ...
    for layer in CHUNK2_LAYERS:
        export_knockout_chunk2(model, {layer}, f"layer{layer}_fp16_only", skip_existing)

    # Two-layer combos
    for a, b in [(7, 8), (8, 9), (9, 10)]:
        export_knockout_chunk2(model, {a, b}, f"layer{a}+{b}_fp16", skip_existing)
...
class DedupEngine:
    """Loads 9 dedup chunks with optional chunk2 override."""

    def __init__(self, chunk2_override=None):
        embed_lmhead = os.path.join(FP32_DIR, "embed_lmhead_combined.mlpackage")
        cu = ct.ComputeUnit.CPU_AND_NE

        print(f"  Loading embed (CPU_ONLY)...")
        self.embed = ct.models.MLModel(embed_lmhead, compute_units=ct.ComputeUnit.CPU_ONLY,
                                       function_name="embed")
        print(f"  Loading lmhead (CPU_ONLY)...")
        self.lmhead = ct.models.MLModel(embed_lmhead, compute_units=ct.ComputeUnit.CPU_ONLY,
                                        function_name="lmhead")
        self.ffns = []
        self.inp_maps = []
        for ci in range(NUM_CHUNKS):
            if ci == 2 and chunk2_override:
                path = chunk2_override
                src = os.path.basename(chunk2_override)
            else:
                path = os.path.join(FP32_DEDUP, f"chunk{ci}.mlpackage")
                src = f"fp32/chunk{ci}"
            print(f"  Loading chunk {ci}: {src} ({cu})")
...
def print_sensitivity_table(all_comparisons):
    print("\n" + "=" * 100)
    print("  PER-LAYER SENSITIVITY TABLE")
    print("=" * 100)
    header = (f"{'Config':<22s} | {'P':>1s} | {'1st':>3s} | {'Early10':>7s} | "
    ...
    for config_name, comparisons, results in all_comparisons:
        for pi, (comp, res) in enumerate(zip(comparisons, results)):
            ...
            print(f"{config_name:<22s} | {pi+1} | {first_tok:>3s} | {early:>7s} | "
        ...
        print(f"{'  (avg)':<22s} |   | {n_first}/3 | {avg_early:>6.0%} | "
...
def print_ranking(all_comparisons):
    ...
    print(f"{'Rank':>4s}  {'Config':<22s}  {'Score':>5s}  {'1stTok':>6s}  {'Early':>6s}  {'Full':>6s}  {'Reps':>4s}  {'Coh':>5s}")
    for rank, (name, score, ae, af, nf, nr, ac) in enumerate(scores, 1):
        print(f"{rank:>4d}  {name:<22s}  {score:>5.1f}  {nf}/3     {ae:>5.0%}  {af:>5.0%}  {nr:>4d}  {ac:>5.3f}")
...
def main():
    ...
    parser.add_argument("--skip-combos", action="store_true", help="Skip 2-layer combo tests")
    args = parser.parse_args()

    print("=" * 70)
    print("  Chunk2 Per-Layer FP16 Knockout — Qwen3.5-4B")
    ...
    configs = [
        ("chunk2_all_fp32",    None),
        ("chunk2_all_fp16",    os.path.join(FP16_DEDUP, "chunk2.mlpackage")),
        ("layer7_fp16_only",   os.path.join(KNOCKOUT_DIR, "layer7_fp16_only.mlpackage")),
        ("layer8_fp16_only",   os.path.join(KNOCKOUT_DIR, "layer8_fp16_only.mlpackage")),
        ("layer9_fp16_only",   os.path.join(KNOCKOUT_DIR, "layer9_fp16_only.mlpackage")),
        ("layer10_fp16_only",  os.path.join(KNOCKOUT_DIR, "layer10_fp16_only.mlpackage")),
    ]
    if not args.skip_combos:
        configs += [
            ("layer7+8_fp16",  os.path.join(KNOCKOUT_DIR, "layer7+8_fp16.mlpackage")),
            ("layer8+9_fp16",  os.path.join(KNOCKOUT_DIR, "layer8+9_fp16.mlpackage")),
            ("layer9+10_fp16", os.path.join(KNOCKOUT_DIR, "layer9+10_fp16.mlpackage")),
        ]
    ...
    single_layer_scores = {}
    for config_name, comparisons, results in all_comparisons:
        if config_name.startswith("layer") and "_only" in config_name:
            layer_num = int(config_name.replace("layer", "").replace("_fp16_only", ""))
            avg_full = np.mean([c["full_agreement"] for c in comparisons])
            avg_early = np.mean([c["early_agreement"] for c in comparisons])
            n_first = sum(1 for c in comparisons if c["first_token_match"])
    ...
    if single_layer_scores:
        ...
        print(f"  Least sensitive layer: {ranked[-1][0]}")
        ...
        if chunk2_fp16_data:
            ...
            if chunk2_full < worst_single_full - 0.1:
                print(f"  -> Multiple layers compound: whole chunk2 should stay FP32")
            elif worst_single_full < 0.90:
    ...
    # ── Save raw data ──
    out_path = os.path.join(KNOCKOUT_DIR, "knockout_results.json")
    save_data = {}
