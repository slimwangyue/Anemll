#!/usr/bin/env python3
"""Probe whether layer-27 cache corruption comes from stateful write/read itself."""
...
def _convert_stateful_probe(cfg: Qwen35Config) -> ct.models.MLModel:
    ...
    return _save_and_reload_cpu(mlmodel, "qwen35_l27_stateprobe_")


def _convert_stateless_probe(cfg: Qwen35Config) -> ct.models.MLModel:
    class Wrapper(torch.nn.Module):
        def forward(self, kv_cache_0, key_states, value_states, current_pos):
            pos = current_pos
            kv = kv_cache_0.clone()
            kv[0:1, :, pos:pos + 1, :] = key_states
            kv[1:2, :, pos:pos + 1, :] = value_states
            key_cache = kv[0:1].squeeze(0)
            value_cache = kv[1:2].squeeze(0)
            return key_cache, value_cache

    wrapper = Wrapper().eval()
    kv_cache_0 = torch.zeros((2, cfg.num_key_value_heads, cfg.state_length, cfg.head_dim), dtype=torch.float16, device=TEST_DEVICE)
    key_states = torch.zeros((1, cfg.num_key_value_heads, 1, cfg.head_dim), dtype=torch.float16, device=TEST_DEVICE)
    value_states = torch.zeros((1, cfg.num_key_value_heads, 1, cfg.head_dim), dtype=torch.float16, device=TEST_DEVICE)
    current_pos = torch.zeros((1,), dtype=torch.int32, device=TEST_DEVICE)
    traced = torch.jit.trace(wrapper, (kv_cache_0, key_states, value_states, current_pos))
    mlmodel = ct.convert(
        traced,
        inputs=[
            ct.TensorType(name="kv_cache_0", shape=kv_cache_0.shape, dtype=np.float16),
            ct.TensorType(name="key_states", shape=key_states.shape, dtype=np.float16),
            ct.TensorType(name="value_states", shape=value_states.shape, dtype=np.float16),
            ct.TensorType(name="current_pos", shape=current_pos.shape, dtype=np.int32),
        ],
    ...
    return _save_and_reload_cpu(mlmodel, "qwen35_l27_statelessprobe_")
...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        key_cache = kv[0:1].squeeze(0)
        value_cache = kv[1:2].squeeze(0)
    print("pt_cache_ready")

    stateful = _convert_stateful_probe(cfg)
    out_stateful = stateful.predict(
        {
    ...
    print(_metric("stateful key_cache", torch.from_numpy(out_stateful["key_cache"]), key_cache))
    print(_metric("stateful value_cache", torch.from_numpy(out_stateful["value_cache"]), value_cache))

    stateless = _convert_stateless_probe(cfg)
    out_stateless = stateless.predict(
        {
            "kv_cache_0": np.zeros((2, cfg.num_key_value_heads, cfg.state_length, cfg.head_dim), dtype=np.float16),
            "key_states": pt_k.numpy(),
            "value_states": pt_v.numpy(),
            "current_pos": current_pos.numpy(),
        }
    )
    print(_metric("stateless key_cache", torch.from_numpy(out_stateless["key_cache"]), key_cache))
    print(_metric("stateless value_cache", torch.from_numpy(out_stateless["value_cache"]), value_cache))
