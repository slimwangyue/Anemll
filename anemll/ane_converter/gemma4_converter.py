"""Converter for Gemma4 models.

This module provides a lightweight converter that mirrors the
:class:`LlamaConverter` behaviour for Gemma4 models without inheriting from
it. Supports Gemma4 architecture with its unique features:
...
from ..models.gemma4_model import (
    Gemma4ForCausalLM,
    Gemma4Config,
    MODEL_DTYPE,
    TEST_DEVICE,
...
class Gemma4Converter(BaseConverter):
    """Handle conversion of Gemma4 270M models to Core ML."""
    ...
    def __init__(
        ...
        # For Gemma4, must be at least sliding_window for local attention to work
    ...
    def load_weights_from_hf(self, hf_model_path: str) -> bool:
        ...
        try:
            ...
            # Gemma4 was trained in bfloat16 which has a larger dynamic range than float16
    ...
    def GetTransformerStates(model, part=None, prefix="model.model.", start_layer=None, end_layer=None):
        ...
        For split cache mode (Gemma4 local/global attention):
        ...
        """
        head_dim_local = getattr(model.config, "head_dim", 256)
        head_dim_global = getattr(model.config, "global_head_dim", head_dim_local)

        # Check if split cache is enabled
        use_split_cache = getattr(model.config, 'use_split_cache', True)

        if use_split_cache:
            # Split cache mode for Gemma4 local/global attention
            # Only count SOURCE layers (non-shared) for KV cache
            first_shared = model.config.num_hidden_layers - getattr(model.config, 'num_kv_shared_layers', 0)
            source_layers = list(range(first_shared))
            num_global_layers = sum(1 for i in source_layers
                                   if model.config.layer_types[i] == "full_attention")
            num_local_layers = len(source_layers) - num_global_layers
            ...
            print(f"  {num_local_layers} source local layers -> kv_cache_local [{2*num_local_layers}, {model.config.num_key_value_heads}, {sliding_window}, {head_dim_local}]")
            print(f"  {num_global_layers} source global layers -> kv_cache_global [{2*num_global_layers}, {model.config.num_key_value_heads}, {model.config.state_length}, {head_dim_global}]")
            ...
            states = [
                ct.StateType(
                    wrapped_type=ct.TensorType(
                        shape=(
                            2 * num_local_layers,  # K and V for source local layers
                ...
                ct.StateType(
                    wrapped_type=ct.TensorType(
                        shape=(
                            2 * num_global_layers,  # K and V for source global layers
                            model.config.num_key_value_heads,
    ...
    def convert(
        ...
        """
        print(f"Gemma4Converter.convert() called with part={part}")
        require_coreml()
        ...
        self.postprocess()
        print("Gemma4Converter.convert() completed")
        return mlmodel

    def convert_to_coreml(self, model: Gemma4ForCausalLM) -> ct.models.MLModel:
        """Convert the entire model to CoreML."""
        require_coreml()
        print("Creating wrapper model...")

        class Wrapper(torch.nn.Module):
            def __init__(self, model: Gemma4ForCausalLM, context_length: int) -> None:
                super().__init__()
                self.model = model
                self.context_length = context_length
        ...
        if is_prefill and not force_rotation:
            if use_update_mask:
                class MonolithicWrapper(torch.nn.Module):
                    ...
                    def __init__(
                        self, model: Gemma4ForCausalLM, context_length: int, is_prefill: bool,
                    ...
                    def forward(
                        ...
                        # Step 1: Embeddings (with Gemma4 scaling)
    ...
    # --------------------------------------------------------------
    def convert_part_1(self, model: Gemma4ForCausalLM) -> ct.models.MLModel:
        """Convert embeddings layer only."""
        require_coreml()
        return self.convert_embeddings(model)

    def convert_part_3(self, model: Gemma4ForCausalLM, argmax_in_model: bool = False) -> ct.models.MLModel:
        """Convert LM head only.

        Args:
            model: The Gemma4 model
        ...
        class LMHeadWrapper(torch.nn.Module):
            def __init__(self, model: Gemma4ForCausalLM, argmax_mode: bool = False) -> None:
                super().__init__()
    ...
    def convert_part_2(
        self, model: Gemma4ForCausalLM, chunk_idx: int = 0, total_chunks: int = 1,
        ...
        Args:
            ...
            start_layer: Optional explicit start layer (overrides auto-calculation)
            end_layer: Optional explicit end layer (overrides auto-calculation)
        """
        require_coreml()
        ...
        if start_layer is not None and end_layer is not None:
            # Explicit layer range provided (for type-separated chunking)
        ...
        class FFNWrapper(torch.nn.Module):
            def __init__(self, model: Gemma4ForCausalLM, start_layer: int, end_layer: int) -> None:
                super().__init__()
                self.model = model  # Use Gemma4ForCausalLM as root
                self.start_layer = start_layer
                self.end_layer = end_layer
                self.states = Gemma4Converter.GetTransformerStates(
                    model, part="2", prefix="model.model.",
                    start_layer=start_layer, end_layer=end_layer
                )

            def forward(self, hidden_states, position_ids, causal_mask, current_pos, per_layer_emb=None):
                # Unflatten PLE if provided: [B, S, total] -> [B, S, num_layers, ple_dim]
                if per_layer_emb is not None:
                    ple_dim = self.model.model.config.hidden_size_per_layer_input
                    num_layers = self.model.model.config.num_hidden_layers
                    bsz, seq_len, _ = per_layer_emb.shape
                    per_layer_emb = per_layer_emb.view(bsz, seq_len, num_layers, ple_dim)
        ...
        # PLE input (flattened per-layer embeddings)
        ple_total_dim = getattr(model.config, 'hidden_size_per_layer_input', 0) * model.config.num_hidden_layers
        per_layer_emb = torch.zeros(
            (1, 1, ple_total_dim), dtype=torch.float16, device=TEST_DEVICE
        ) if ple_total_dim > 0 else None

        self._reset_kv_cache_buffers(wrapper)
        trace_inputs = (hidden_states, position_ids, causal_mask, current_pos)
        if per_layer_emb is not None:
            trace_inputs = trace_inputs + (per_layer_emb,)
        traced = torch.jit.trace(
            wrapper, trace_inputs
        )
        self._reset_kv_cache_buffers(wrapper)
        self._reset_kv_cache_buffers(traced)
        ...
        mlmodel = ct.convert(
            ...
            inputs=[
                ...
                ct.TensorType(
                    ...
                    name="per_layer_emb", shape=per_layer_emb.shape, dtype=np.float16
            )] if per_layer_emb is not None else []),
            outputs=[ct.TensorType(name="output_hidden_states", dtype=np.float16)],
            states=self.GetTransformerStates(model, part=None, prefix="model.model.",
                                             start_layer=start_layer, end_layer=end_layer),
            compute_precision=ct.precision.FLOAT16,
    ...
    def convert_part_2_prefill(
        ...
        if use_update_mask:
            class PrefillWrapper(torch.nn.Module):
                def __init__(self, model: Gemma4ForCausalLM, start_layer=0, end_layer=None,
    ...
    def convert_prefill(self, model: Gemma4ForCausalLM) -> ct.models.MLModel:
        """Convert Gemma4 model to CoreML format for prefill mode.
        ...
        """
        require_coreml()
        print("Converting Gemma4 model for prefill mode...")
        ...
        if use_update_mask:
            class PrefillWrapper(torch.nn.Module):
                def __init__(
                    self, model: Gemma4ForCausalLM, context_length: int, batch_size: int
                ) -> None:
                    super().__init__()
                    self.model = model
                    self.context_length = context_length
    ...
    def convert_embeddings(self, model: Gemma4ForCausalLM) -> ct.models.MLModel:
        """Convert embeddings layer to CoreML format.

        Args:
            model: The Gemma4 model containing embeddings

        Returns:
            ct.models.MLModel: Converted CoreML model for embeddings
        """
        require_coreml()
        print("\nConverting Gemma4 embeddings layer...")
        ...
        class EmbeddingsWrapper(torch.nn.Module):
            def __init__(self, model):
                ...
                self.has_ple = hasattr(model.model, 'embed_tokens_per_layer')
                if self.has_ple:
                    self.embed_tokens_per_layer = model.model.embed_tokens_per_layer
                    self.per_layer_model_projection = model.model.per_layer_model_projection
                    self.per_layer_projection_norm = model.model.per_layer_projection_norm
                    self.ple_scale = model.model.ple_scale
            ...
            def forward(self, input_ids):
                ...
                if self.has_ple:
                    per_layer_emb = model.model.compute_per_layer_embeddings(
                        input_ids, inputs_embeds=hidden_states)
                    # Flatten PLE: [B, S, num_layers, ple_dim] -> [B, S, num_layers*ple_dim]
                    bsz, seq_len = per_layer_emb.shape[:2]
                    per_layer_emb = per_layer_emb.reshape(bsz, seq_len, -1).to(MODEL_DTYPE)
                    return hidden_states, per_layer_emb
        ...
        mlmodel = ct.convert(
            ...
            outputs=[
                ...
                ct.TensorType(name="per_layer_emb", dtype=np.float16),
            ],
            compute_precision=ct.precision.FLOAT16,
            compute_units=ct.ComputeUnit.CPU_AND_NE,
            minimum_deployment_target=ct.target.iOS18,
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Convert Gemma4 model to CoreML format")

    parser.add_argument(
        "--model",
        type=str,
        help="Path to model directory (default: google/gemma-4n-E2B-it)",
    )
    parser.add_argument(
        "--prefix",
        type=str,
    ...
    parser.add_argument(
        "--single-cache",
        action="store_true",
        help="Use a single unified KV cache instead of split local/global caches (Gemma4 only).",
    )
    parser.add_argument(
        "--part",
        type=str,
    ...
    parser.add_argument(
        "--decorate",
        action="store_true",
        help="Add CTX and ATT values to output filename (e.g., gemma4_monolithic_CTX512_ATT64.mlpackage)",
    )
    parser.add_argument(
        "--fp16-scale",
        type=str,
...
# Recommended FP16 scaling factors for Gemma4 models
# Based on residual stream analysis - see docs/GEMMA3_FP16_SCALING.md
GEMMA4_SCALING_FACTORS = {
    # Model identifier patterns -> recommended alpha
    # Gemma4 E4B has fp16 overflow in MLP down_proj (peak ~316K >> 65504)
    "gemma-4-E4B": 0.20,       # Peak ~316K in down_proj (4.8x FP16 max)
    "gemma4-E4B": 0.20,
    "gemma-4-E2B": 0.5,        # Estimate, needs verification
    "gemma4-E2B": 0.5,
}
...
def _get_auto_scale_factor(model_path: str) -> Optional[float]:
    ...
    for pattern, alpha in GEMMA4_SCALING_FACTORS.items():
...
def _apply_fp16_scaling(
    ...
    2. Transform post-norm weights: w_new = α * w_old
    ...
    Args:
        model: The Gemma4 model with loaded weights
    ...
    if fp16_scale.lower() == "auto":
        ...
        if alpha is None:
            ...
            print(f"      Known models: {list(GEMMA4_SCALING_FACTORS.keys())}")
    ...
    # Gemma4 RMSNorm uses: output = norm(x) * weight (direct, not 1+weight)
    # To scale the gain by α: w_new = α * w_old
    scaled_layers = 0
...
def test_conversion(
    model: Optional[Gemma4ForCausalLM] = None,
    ...
    """Convert a Gemma4 model and save the result.

    Args:
        model: Pre-loaded Gemma4 model (optional)
    ...
    if model is None:
        ...
        # For Gemma4, context_length must be >= sliding_window for local attention to work
        ...
        # Set attention_size for ANE optimization (separate from Gemma4's sliding_window)
        # sliding_window stays unchanged - it's Gemma4's architectural feature
        ...
        if attention_size is not None:
            ...
            # For Gemma4, attention_size must be >= sliding_window for local attention
        ...
        model = Gemma4ForCausalLM(config, enable_coreml=True)
...
def main() -> None:
    print("Starting gemma4_converter main()...")
    args = parse_args()
    ...
    model_path = args.model if args.model else "google/gemma-4n-E2B-it"
